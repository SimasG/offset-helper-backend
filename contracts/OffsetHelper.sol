...
contract OffsetHelper is OffsetHelperStorage {
        ...
        returns (address[] memory tco2s, uint256[] memory amounts)
    {
        ...
        swapExactOutETH(_poolToken, _amountToOffset, customPath);
    ...
    function autoOffsetExactInETH(
        address _poolToken,
        // technically, `customPath` should always be [] here. Solidity doesn't support default paratemers yet though.
        address[] memory customPath
    )
        public
        payable
    ...
    {
        // swap MATIC for BCT / NCT
        uint256 amountToOffset = swapExactInETH(_poolToken, customPath);
    ...
    function swapExactOutToken(
        ...
        uint256 _toAmount,
        address[] memory customPath
    ) public onlyRedeemable(_toToken) {
        // calculate path & amounts
        (
            address[] memory path,
            uint256[] memory expAmounts
        ) = calculateExactOutSwap(_fromToken, _toToken, _toAmount, customPath);
    ...
    function swapExactInToken(
        ...
        // calculate path & amounts
        address[] memory path = generatePath(_fromToken, _toToken, customPath);
    ...
    function calculateExactOutSwap(
        ...
        path = generatePath(_fromToken, _toToken, customPath);
        uint256 len = path.length;
     ...
     */
    // ** Not used anywhere here. What's the purpose of it?
